...
type MediaFileController struct {
	*gc.Controller
	service services.MediaFileService
}

func NewMediaFileController(logger *zap.Logger, s services.MediaFileService) *MediaFileController {
...
func (ctl *MediaFileController) Upload(c *gin.Context) {
	gc.JsonData(c, nil)

}

func (ctl *MediaFileController) GetSign(c *gin.Context) {
	sign, err := ctl.service.GetSign(c.Request.Context())
	if gc.HandleError(c, err) {
		ctl.Logger.Error("Get sign error", zap.Error(err))
		return
	}
...
func (ctl *MediaFileController) UploadCallback(c *gin.Context) {

	fmt.Println("\nHandle Post Request ... ")

	// Get PublicKey bytes
	bytePublicKey, err := getPublicKey(c)
	if gc.HandleError(c, err) {
		ctl.Logger.Error("Get sign error", zap.Error(err))
		return
	}
	...
	byteAuthorization, err := getAuthorization(c)
	if gc.HandleError(c, err) {
		ctl.Logger.Error("Get sign error", zap.Error(err))
		return
	}
	...
	byteMD5, err := getMD5FromNewAuthString(c)
	if gc.HandleError(c, err) {
		ctl.Logger.Error("Get sign error", zap.Error(err))
		return
	}

	ok := ctl.service.UploadCallback(c.Request.Context(), bytePublicKey, byteMD5, byteAuthorization)
	if ok {
...
func getMD5FromNewAuthString(c *gin.Context) ([]byte, error) {
	...
	strURLPathDecode, errUnescape := unescapePath(c.Request.URL.Path, encodePathSegment) //url.PathUnescape(r.URL.Path) for Golang v1.8.2+
	if errUnescape != nil {
